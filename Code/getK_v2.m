...
% errTol is the error tolerance for the resulting K.
% lastK is the initial guess of K.
% FPS is frames per second.
% ArcLength is the length of the worm in mm (10^(-3) meters).
	% If ArcLength is 0, it means the input coordinates are already given in mm so they will NOT be converted.
	% The arc length of the worm in mm and will be used to set the segment length (Ls)...
...
function [Kout,D_error,Success] = getK_v2(allX,allY,errTol,lastK,FPS,ArcLength)
  ...
  if(ArcLength) % If ArcLength>0, convert unit length such that the total length is equal to ArcLength.
    L = zeros(1,Nt);
    L_m = 1e-3;
    for i = 1:Nt
        L(i) = 0;
